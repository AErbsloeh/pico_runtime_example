...
uint8_t AD528x_get_adress(ad528x_device_t *handler){
...
uint8_t AD528x_create_command(ad528x_device_t *handler){
    return ((handler->shutdown) ? 0x00 : 0x20) | ((handler->gpio_state && AD528X_GPIO_PIN0) << 0x10) | ((handler->gpio_state && AD528X_GPIO_PIN1) << 0x08);
}


bool AD528x_write_data(ad528x_device_t *handler, uint8_t command, uint8_t data){
...
bool ad528x_define_output(ad528x_device_t *handler, bool chnnl_b, uint8_t position){
