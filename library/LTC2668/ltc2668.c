...
int8_t handler_pico_spi_transmission(ltc2668_handler_t *handler, uint8_t cmd, uint8_t adr, uint16_t data){
...
bool ltc2668_init(ltc2668_handler_t *config_device){
    ...
    //handler_pico_spi_transmission(config_device, CMD_PWR_DWN_ALL, 0x00, 0x0000);
    sleep_ms(10);

    // --- Controlling the reference device
    if(config_device->use_int_vref){
        handler_pico_spi_transmission(config_device, CMD_CONFIG, 0x00, 0x0000);
    } else {
        handler_pico_spi_transmission(config_device, CMD_CONFIG, 0x00, 0x0001);
    };
    sleep_ms(10);
    
    // --- Power-Up device
    if(config_device->pwr_up_chnnl == 0xFFFF){
        handler_pico_spi_transmission(config_device, CMD_UPD_DAC_ALL, 0x00, 0x0000);
        sleep_us(100);
        ...
        for(uint8_t idx = 0; idx < 16; idx++){
            ...
            if(activate_chnnl){
                handler_pico_spi_transmission(config_device, CMD_UPD_DAC_N, idx, 0x0000);
                sleep_us(10);
            };
        };
    };    
    ...
    for(uint8_t chnl=0; chnl < 16; chnl++){
        handler_pico_spi_transmission(config_device, CMD_WR_SPAN_DAC_N, chnl, config_device->vref_range);
        sleep_us(10);
    }  

    ltc2668_clear_data_soft(config_device);
    sleep_us(100);  
...
void ltc2668_update_vrange(ltc2668_handler_t *config_device, uint8_t chnl, uint8_t vref_mode){
    handler_pico_spi_transmission(config_device, CMD_WR_SPAN_DAC_N, chnl, vref_mode);
    sleep_us(10);
}
...
void ltc2668_clear_data_soft(ltc2668_handler_t *config_device){
    for(uint8_t chnl=0; chnl < 16; chnl++){
        handler_pico_spi_transmission(config_device, CMD_WR_CODE_DAC_N, chnl, 0x8000);
        sleep_us(5);
    };        
}
...
void ltc2668_mux_control(ltc2668_handler_t *config_device, bool enable, uint8_t channel){
    ...
    handler_pico_spi_transmission(config_device, CMD_MUX_CONTROL, 0x00, mux_data);
}


void ltc2668_span_all_control(ltc2668_handler_t *config_device, uint8_t span){
    uint16_t data = span & 0x07; // mask Bit[2:0]
    handler_pico_spi_transmission(config_device, CMD_WR_SPAN_DAC_ALL, 0x00, data);
}


void ltc2668_write_output_all_channel(ltc2668_handler_t *config_device, uint16_t data){
    handler_pico_spi_transmission(config_device, CMD_WR_CODE_DAC_ALL, 0x00, data);
}


void ltc2668_write_output_single_channel(ltc2668_handler_t *config_device, uint16_t data, uint8_t channel){
    handler_pico_spi_transmission(config_device, CMD_WR_CODE_DAC_N, channel & 0x0F, data);
}
...
void ltc2668_update_output_all_channel(ltc2668_handler_t *config_device, uint16_t data){
    ...
    handler_pico_spi_transmission(config_device, CMD_WR_UPD_DAC_ALL, 0x00, data_real);
}


void ltc2668_update_output_single_channel(ltc2668_handler_t *config_device, uint16_t data, uint8_t channel){
    uint16_t data_real = (config_device->use_16bit_dev) ? data : (data & 0x0FFF) << 4;
    handler_pico_spi_transmission(config_device, CMD_WR_UPD_DAC_N, channel & 0x0F, data_real);
}
