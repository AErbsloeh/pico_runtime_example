...
from generator.helper.metric_data import (calculate_error_mae, calculate_error_mbe,
                                                 calculate_cosine_similarity, calculate_total_harmonics_distortion)
...
def generation_sinusoidal_waveform(bitsize_lut: int, f_rpt: float, f_sine: float,
                                   bitsize_chck: int=-1, do_signed: bool=False, do_optimized: bool=False) -> np.ndarray:
    ...
    offset = 0 if do_signed or do_optimized else 1
    ...
    sine_lut_chck = checking_binary_limits_violation(sine_lut, bitsize_lut + (-1 if do_optimized else 0), do_signed)
    return np.array(sine_lut_chck, dtype=np.int32)


def generate_triangular_waveform(f_sig: float, fs: float, no_periods: int=1, bit: int=16) -> [np.ndarray, np.ndarray]:
...
def generate_rectangular_waveform(f_sig: float, fs: float, no_periods: int=1, bit: int=16) -> [np.ndarray, np.ndarray]:
